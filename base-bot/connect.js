    ...
    ask.ev.on('group-participants.update', async ({ id, participants, action }) => {
        ...
        if (action === 'demote' && groupSettings[id]?.antidemote) {
            try {
                ...
                await ask.sendMessage(id, {
                    text: `🛡️ Une rétrogradation a été bloquée (anti-demote activé)`,
        ...
        if (action === 'promote' && groupSettings[id]?.antipromote) {
            try {
                ...
                await ask.sendMessage(id, {
                    text: `🛡️ Action bloquée : @${adminWhoTried.split('@')[0]} a tenté de promouvoir des membres (antipromote activé)`,
...
ask.ev.on('group-participants.update', async (event) => {
    ...
    ask.ev.on("groups.update", async (updates) => {
        ...
        try {
            ...
            const messages = {
                announce: {
                    true: "Le groupe a été verrouillé, seuls les admins peuvent envoyer des messages",
                    false: "Le groupe a été déverrouillé, tous les membres peuvent envoyer des messages"
                },
                restrict: {
                    true: "Les infos du groupe sont restreintes, seul l'admin peut les modifier",
                    false: "Les infos du groupe sont ouvertes, tous les membres peuvent les modifier"
                }
            };

            if (res.announce !== undefined) {
                await ask.sendMessage(res.id, {
                    text: `「 Paramètres du groupe mis à jour 」\n\n${messages.announce[res.announce]}`
                });
            } 
            else if (res.restrict !== undefined) {
                await ask.sendMessage(res.id, {
                    text: `「 Paramètres du groupe mis à jour 」\n\n${messages.restrict[res.restrict]}`
                });
            }
            else if (res.desc) {
                await ask.sendMessage(res.id, { 
